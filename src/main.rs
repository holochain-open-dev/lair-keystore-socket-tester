...
async fn main() {
    ...
    let socket_url_parsed = Url::parse(args.socket_url.clone().as_str())
        .expect(format!("socket_url {:?} is not a valid URL", args.socket_url.clone()).as_str());
    let password_bufread: BufRead = args.password
        .as_bytes()
        .into();
    ...
    println!("Successfully connected to lair keystore at {:?}", args.socket_url);
}
